...
Workers must be able to reach runtime node IPs on TCP `2375`. The base NetworkPolicy allows that egress port, but production clusters should also restrict the host port with private networking, node firewall rules or security groups. Token authentication and the Docker API allowlist are additional controls; Docker API port `2375` must never be publicly exposed.
