...
Execution worker container limits protect the worker process. `EXECUTION_WORKER_WORKLOAD_CONTAINER_*` limits protect each Docker workload container launched by the worker, and do not apply to image pulls.
...
Replicas that share a Docker host coordinate identical image pulls through Redis. Different hosts still pull independently. Build completion records the resolved workflow image digest; execution workers ensure that digest on the assigned runtime node before creating the container.
