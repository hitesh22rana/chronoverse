...
const (
	// MaxRetries is the maximum number of retries for the health check
	MaxRetries = 3
)
...
func NewRedisStore(cfg *RedisConfig) (*RedisStore, func() error, error) {
	...
	// Set the max memory
	if err := client.ConfigSet(
		ctx,
		"maxmemory",
		cfg.MaxMemory,
	).Err(); err != nil {
		return nil, nil, fmt.Errorf("failed to set max memory: %v", err)
	}
	...
	if err := client.ConfigSet(
		...
		return nil, nil, fmt.Errorf("failed to set eviction policy: %v", err)
	}
	...
	if err := client.ConfigSet(
		ctx,
		"maxmemory-samples",
		fmt.Sprintf("%d", cfg.EvictionPolicySampleSize),
	).Err(); err != nil {
		return nil, nil, fmt.Errorf("failed to set eviction policy sample size (maxmemory-samples): %v", err)
	}
	...
	return &RedisStore{client: client}, func() error {
		return client.Close()
	}, nil
}

// Set stores a value with optional expiration
func (rs *RedisStore) Set(
	ctx context.Context,
	key string,
	value interface{},
