...
For Kubernetes, pre-create `chronoverse-auth`, `chronoverse-ca`, `chronoverse-client-tls`, `chronoverse-service-tls`, `chronoverse-infra-tls`, `chronoverse-kafka-tls`, `postgres-secret`, `clickhouse-secret`, and `meilisearch-secret` when you need to bring your own material. Otherwise `scripts/k8s/setup.sh --mode production` generates missing Secrets.
