...
`FailJob` requires a failure-family terminal reason code, while `CancelClaimedJob` and canceled `UpdateJobStatus` calls require a cancellation-family code. Generic status updates cannot set `FAILED`; workers must use `FailJob` so failure kind, diagnostics, status, and reason are recorded together. Job list and public detail responses expose only the normalized safe code and message, never the stored raw error.
