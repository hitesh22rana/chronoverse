...
Workload egress is filtered on the host: the bridge is pinned to a fixed subnet (`EXECUTION_WORKER_WORKLOAD_SUBNET`, default `198.18.247.0/24`) and a `workload-firewall` unit (Compose service, Kubernetes sidecar in the `docker-proxy` pod) drops host, private, link-local/metadata, and cluster destinations in both the forwarded and host-input packet paths, while public internet and DNS keep working. Admission is gated twice: the execution-worker starts only once the firewall reports healthy, and the runtime-agent withholds node registration while the firewall's ready marker is missing or stale. Shared-disk exhaustion stays operator-owned — there is no image-storage cap, prune job, or per-user image quota.
