...
Execution worker container limits protect the worker process. `EXECUTION_WORKER_WORKLOAD_CONTAINER_*` limits protect each Docker workload container launched by the worker, and do not apply to workflow-worker image pulls.

Execution workers do not need to run on the same node as the Docker daemon. They receive `runtime_endpoint` from `jobs-service` and use the direct Docker data plane for container start, logs, and cleanup.
...
Replicas that share a Docker host coordinate identical image pulls through Redis. Different hosts still pull independently. Build completion records the resolved workflow image digest but does not imply that every runtime node has already cached the image.
