...
- Restrict Docker proxy TCP `2375` to trusted Chronoverse traffic with private networking, node firewall rules or security groups. Preserve the generated proxy token and exact Docker API allowlist; NetworkPolicy support for node-IP hostPort traffic depends on the CNI.
