...
#
# Workload-source rules match the bridge interface (-i), not just the subnet:
# the v6 sources are shared link-local space, and -i keeps the rules off
# unrelated containers even if the subnet is ever reused. The interface name
# is fixed at network creation (never resolved here: resolution would deadlock
# fresh installs, with the firewall waiting for the bridge while the worker
# waits for the firewall). Interface-free rules (-d drops, ESTABLISHED) are
# verdict-identical with or without -i, so probes stay free of host specifics.
#
...
# Deny-before-allow: drop the terminal accept first so added ranges can never
# land after it (shadowed but "healthy"), then re-append it last.
# Removing it only ever fails closed (traffic falls through to Docker's DROP).
iptables -D "$CHAIN" -s "$SUBNET" -i "$BRIDGE_IF" -j ACCEPT 2>/dev/null || true

# Forwarded path: replies, infra drops, then DNS.
# ESTABLISHED stays interface-free on purpose: return traffic ingresses on
# external interfaces, and it can only ever accept replies, never new flows.
...
# Source-subnet match is already precise here (only bridge-ingress or spoofed
# packets carry it, and both are dropped), so no interface match is needed.
rule_in -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
rule_in -s "$SUBNET" -m conntrack --ctstate NEW -j DROP

# IPv6: workloads can only source link-local, which other containers share —
# hence the interface match, and no ACCEPT (RETURN leaves the rest to Docker).
...
# Link jumps only once the chains are complete: no empty-chain window.
