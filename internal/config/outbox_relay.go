...
type OutboxRelayConfig struct {
	...
	IdempotencyCleanupMaxBatches int           `envconfig:"OUTBOX_RELAY_IDEMPOTENCY_CLEANUP_MAX_BATCHES" default:"10"`
...
func InitOutboxRelayConfig() (*OutboxRelay, error) {
	...
	if err := cfg.OutboxRelayConfig.validate(); err != nil {
		return nil, err
	}
	return &cfg, nil
}
...
func (c *OutboxRelayConfig) validate() error {
	if c.CleanupEnabled && c.IdempotencyCleanupMaxBatches <= 0 {
		return errors.New("OUTBOX_RELAY_IDEMPOTENCY_CLEANUP_MAX_BATCHES must be positive when cleanup is enabled")
	}
	return nil
}
