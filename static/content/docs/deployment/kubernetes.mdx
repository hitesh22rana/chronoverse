...
Container workflows execute through runtime ownership. Each labeled Docker-capable node runs one `docker-proxy` DaemonSet pod with a `runtime-agent` sidecar. Official overlays register `tcp://$(NODE_IP):2376` via `hostPort:2376` (`tcp://docker-proxy:2375` must not be persisted); health probes use `tcp://127.0.0.1:2376` while the advertised endpoint remains `tcp://$(NODE_IP):2376`. The proxy binds `:2376 ssl crt /certs/docker-proxy/server.pem ca-file /certs/docker-proxy/ca.crt verify required` plus `X-Chronoverse-Docker-Proxy-Token` and an exact Docker method/path allowlist; per-client mTLS (`DOCKER_PROXY_TLS_*`, `ServerName docker-proxy` via `WithDockerProxyTLS`/`WithDockerProxyToken`) authenticates callers — workload containers have neither.
