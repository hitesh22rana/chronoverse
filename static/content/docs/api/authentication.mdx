...
`POST /auth/register` accepts email and password, creates the user, and issues session and CSRF cookies. Emails are trimmed and lowercased; registration passwords need 8–72 characters with 3 of 4 character classes (lowercase, uppercase, digit, symbol) and must not be common.
