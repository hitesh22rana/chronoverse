# API overview

The public HTTP API is served directly at `http://localhost:8080` in development. Production Nginx exposes the same routes below `/api`.

## Authentication model

Application routes use an encrypted session cookie. Mutating routes additionally validate CSRF state. The gateway exchanges browser identity for signed service metadata on internal gRPC calls.

## Content type

Send `Content-Type: application/json` for JSON request bodies. SSE log streams use `text/event-stream`, and raw log downloads use `text/plain`.

## Safe retries

The following commands require `Idempotency-Key`:

- `POST /workflows`
- `PUT /workflows/{workflow_id}`
- `POST /workflows/{workflow_id}/jobs/schedule`

Reuse a key only when retrying the same logical command.

## Reference

The [endpoint reference](/docs/api/reference) is generated from the repository's OpenAPI document. It covers paths, parameters, request bodies, and response status codes.

<Callout title="Ingress rate limits" type="info">
The production Nginx ingress applies per-client limits: auth endpoints allow 10 requests per minute (burst 5), other routes 20 requests per second (burst 40), and request bodies are capped at 1 MiB. Exceeding a rate or connection limit returns `429`; oversized bodies return `413`. Immutable build assets under `/_next/static/` are exempt from request-rate limiting, and the server-sent events endpoint is connection-capped (5 per client) instead of rate-limited.
</Callout>

