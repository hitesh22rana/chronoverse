...
Each service signs JWTs with its own Ed25519 private key and a `kid` header. The gateway and every verifier mount `certs/issuers/<issuer>/auth.ed` (0440, `fsGroup: 101`) plus a shared `certs/issuers/trusted.json` bundle of public keys; `Auth.ValidateToken` requires the `kid` and verifies the signature only against the matching bundle entry with issuer binding. New builds inject the per-issuer paths through `Makefile` ldflags (for example `certs/issuers/users-service/auth.ed`) and both Compose profiles generate the per-issuer keys and `trusted.json` in `init-certs`, gating startup on the bundle. Kubernetes generates the same layout in `cert-bootstrap` and projects the bundle and each `issuers/<svc>/auth.ed.pub` via mounted volumes. Rotation is graceful: `scripts/compose/rotate-auth-key.sh <issuer>` adds a new key alongside the old `kid` in the bundle, verifiers reload the dual-key bundle before the signer switches to the new `kid`, and the old `kid` is pruned after its 15-minute expiry window (verifiers must restart to drop it); `make compose/validate` guards the wiring.
