...
# The terminal drop is removed first like above: a re-apply must never append
# allows after it.
rule_in "$IPT" -i "$BRIDGE_IF" -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
"$IPT" -D "$CHAIN_IN" -s "$SUBNET" -m conntrack --ctstate NEW -j DROP 2>/dev/null || true
rule_in "$IPT" -i "$BRIDGE_IF" -s "$SUBNET" -p udp --dport 53 -j ACCEPT
rule_in "$IPT" -i "$BRIDGE_IF" -s "$SUBNET" -p tcp --dport 53 -j ACCEPT
