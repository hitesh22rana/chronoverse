...
Scheduler settings control poll interval, context timeout, and batch size. Workflow worker settings control image-pull locks. Execution worker settings control concurrency, per-workload Docker container memory/CPU/PID limits, leases, retries, recovery, and log publication. Processor settings control batching and cleanup. Outbox settings control enabled topic groups, claims, retries, cleanup, and retention.
