...
The Redis lock key includes the runtime node scope and exact image string. Workers sharing a runtime node coordinate; workers on different runtime nodes may pull in parallel. Docker host is used only as a fallback for legacy/local clients that do not pass an explicit runtime scope.
