...
`POST /auth/register` accepts email and password, creates the user, and issues session and CSRF cookies. Emails are trimmed and lowercased; registration passwords need 8–72 characters with a zxcvbn score of at least 3 and must not match the bundled common-password dictionary.
