...
`make test/short` and `make test` run the unit suites (the full target adds the race detector). `make test/integration` runs the Testcontainers-backed integration suite under `internal/repository` against real PostgreSQL, ClickHouse, Redis, Meilisearch, and Kafka; it requires a running Docker daemon.
