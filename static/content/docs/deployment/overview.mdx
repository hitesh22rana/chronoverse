...
`infra/k8s/overlays/production` provides the Kubernetes deployment shape for externally managed stateful dependencies and pre-created Secrets. `infra/k8s/overlays/local` provides a self-contained single-node Kubernetes profile for validation.
...
Compose remains the quickest full-stack path. Kubernetes support is provided through Kustomize and expects production operators to own managed storage, secret lifecycle, ingress hostnames, and Docker-capable worker nodes.
