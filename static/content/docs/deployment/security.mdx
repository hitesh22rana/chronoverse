...
Execution components use a socket proxy with a narrow API surface instead of broad direct daemon access. The proxy permits image, container, and log operations plus exactly two network operations — inspect and create — solely so the runtime can guarantee the dedicated `chronoverse-workloads` isolation network exists (bridge driver, inter-container communication disabled) before any workload runs; startup fails if it cannot.
