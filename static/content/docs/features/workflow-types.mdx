...
Heartbeat targets must be publicly reachable endpoints. Requests that resolve to loopback, private, link-local, or multicast addresses are rejected at execution time and fail the job permanently with `EXECUTION_FAILED`. The check is applied per connection attempt, so redirects and DNS changes cannot route a probe to an internal address.
